...
abstract class SiteRiskPoaDao : BaseDao<SiteRiskPoaEntity> {
    ...
    @Query("UPDATE SiteRiskPoaEntity SET closureProofAttachment=1, closureDateTime= :closureDate, poAStatus=:status, closingRemarks=:remarks, siteStatusOnCloseTime=:siteStatusOnCloseTime, isSynced=0 WHERE id = :poaId")
    abstract fun updateOnClosingPOA(
        closureDate: String,
        remarks: String,
        status: Int,
        poaId: Int,
        siteStatusOnCloseTime: Int,
    ): Maybe<Int>

    @Query("SELECT id,siteRiskId,closureDateTime,closingRemarks,closureProofAttachment,siteStatusOnCloseTime from SiteRiskPoaEntity where poAStatus=3 and isSynced=0")
