...
import com.sisindia.ai.android.mlcore.qrdetection.PreferenceUtils;
...
public class QRCameraSource {
    ...
    private final QRGraphicOverlay graphicOverlay;
    ...
    private QRFrameProcessor frameProcessor;

    public QRCameraSource(QRGraphicOverlay graphicOverlay) {
    ...
    private static CameraSizePair selectSizePair(Camera camera, float displayAspectRatioInLandscape) {
        List<CameraSizePair> validPreviewSizes = MlUtils.generateValidPreviewSizeList(camera);
        ...
        for (CameraSizePair sizePair : validPreviewSizes) {
            ...
            if (Math.abs(aspectRatioDiff - minAspectRatioDiff) < MlUtils.ASPECT_RATIO_TOLERANCE) {
        ...
        if (selectedPair == null) {
            ...
            for (CameraSizePair sizePair : validPreviewSizes) {
                ...
                int diff =
                        Math.abs(size.getWidth() - QRCameraSource.DEFAULT_REQUESTED_CAMERA_PREVIEW_WIDTH)
                                + Math.abs(size.getHeight() - QRCameraSource.DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT);
                if (diff < minDiff) {
                    selectedPair = sizePair;
                    minDiff = diff;
                }
    ...
    public void setFrameProcessor(QRFrameProcessor processor) {
        graphicOverlay.clear();
        synchronized (processorLock) {
            if (frameProcessor != null) {
                frameProcessor.stop();
    ...
    private void setPreviewAndPictureSize(Camera camera, Parameters parameters) throws IOException {
        ...
        CameraSizePair sizePair = PreferenceUtils.getUserSpecifiedPreviewSize();
        if (sizePair == null) {
            ...
            if (MlUtils.isPortraitMode(graphicOverlay.getContext())) {
        ...
        Timber.v("Camera preview size: " + previewSize);
        parameters.setPreviewSize(previewSize.getWidth(), previewSize.getHeight());
        Prefs.putString(PREF_KEY_REAR_CAMERA_PREVIEW_SIZE, previewSize.toString());
        ...
        if (pictureSize != null) {
            ...
            Prefs.putString(PREF_KEY_REAR_CAMERA_PICTURE_SIZE, pictureSize.toString());
        }
    }
