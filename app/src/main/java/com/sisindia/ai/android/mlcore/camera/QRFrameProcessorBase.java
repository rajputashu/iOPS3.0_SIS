package com.sisindia.ai.android.mlcore.camera;
...
/**
 * Abstract base class of {@link QRFrameProcessor}.
 */
public abstract class QRFrameProcessorBase<T> implements QRFrameProcessor {

//    private static final String TAG = "QRFrameProcessorBase";
    ...
    @Override
    public synchronized void process(ByteBuffer data, FrameMetadata frameMetadata, QRGraphicOverlay graphicOverlay) {
        latestFrame = data;
    ...
    private synchronized void processLatestFrame(QRGraphicOverlay graphicOverlay) {
...
//                                Log.d(TAG, "Latency is: " + (SystemClock.elapsedRealtime() - startMs));
                QRFrameProcessorBase.this.onSuccess(image, results, graphicOverlay);
                processLatestFrame(graphicOverlay);
            }).addOnFailureListener(QRFrameProcessorBase.this::onFailure);
        }
    }
     ...
     */
    protected abstract void onSuccess(FirebaseVisionImage image, T results, QRGraphicOverlay graphicOverlay);
